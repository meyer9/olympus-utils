...
func TestExtendedPrivateKey(t *testing.T) {
	...
	expectedPub := g1pubs.PrivToPub(priv)

	if !expectedPub.Equals(*childPub) {
...
func TestExtendedKeyToFromString(t *testing.T) {
	...
	gotPrivBytes := gotPriv.Serialize()
	expectedPrivBytes := expectedPriv.Serialize()
	...
	gotPubBytes := gotPub.Serialize()
	expectedPubBytes := expectedPub.Serialize()
